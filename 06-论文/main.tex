...
由干固体守恒、水分守恒与能量守恒导出，以干物质标签 $q$ 为材料坐标，编写成质量坐标
形式，收缩几何下形式不变，且无伪对流项；求解用守恒型有限体积（表面半单元串联阻力、
...
\smallskip\noindent\textbf{验证与创新}：验证分六层，每层都说明能排除的与不能排除的错误
类型；解析解与数值解是两种不同数学结构的互相验证；已检查因素的累计偏移量级约 1.71 h
...
力学驱动扩展 RMSE 0.522 mm、$t_f=51.1667$ h，结构性上限与材料缺口均已从数据上说明。
...
给出（0--14400 s、60 s 一步），药材失水后伴随着外半径收缩（附件 2，0--72 h、
1800 s 一步，由 2.0 cm 缩至 1.198 cm 后进入近平台段）。烘干达标的定义为药材各处
...
\begin{enumerate}
  \item[\textbf{问题 1}] 预热平衡阶段（0--1800 s）药材内温度与水分浓度的时空分布
        （附录 2 的材料参数），按题目所给的表格格式提交（result1.xlsx）；
  \item[\textbf{问题 2}] 整个烘干过程的温度与水分浓度分布情况（附录 3 的材料参数，result2.xlsx）；
  \item[\textbf{问题 3}] 固定几何下首次全部达标的烘干时间（表 5、result3.xlsx）；
  \item[\textbf{问题 4}] 计入失水收缩（附件 2 外半径、附录 4 的材料参数）后的达标时间
        （表 6、result4.xlsx）。
\end{enumerate}
总体思路是建立统一模型，用\textbf{一套热湿耦合控制方程加四个
配置开关}（材料参数来源、热湿耦合、几何是否演化、输出形式）统一求解：问题 1 是
问题 2 的解耦特例，问题 3 是问题 2 加阈值判据，问题 4 是移动域的推广；对收缩几何，
先以物理启发的半经验闭合，由内而外地预测半径（附件 2 降为验证数据），再用力学驱动
扩展模型讨论收缩的机制边界。几何模型与边界条件如示意见图~\ref{fig:几何示意}。
...
四个问题（以下简称四问）共享同一物理过程：内部导热/导湿、表面对流边界、材料参数强非线性。难点有三个方面：
...
强非线性，尾段 $D$ 最多缩小到原先的 16.8 倍，形成低扩散率"干壳"，决定达标时间的尾段行为；
\textbf{(iii)} 环境数据仅有 4 h、半径数据 72 h 而干燥则需 50 h 以上，边界外推与
收缩闭合的取值选择直接改变结论量级。
...
按附录 2 为常数材料参数）。建立线性热传导的解析谱解（圆柱 Robin 特征展开 + 时变环境
Duhamel 卷积）作为独立参考，数值解与之逐点比较，形成排除离散、边界、时间积分
...
仿射收缩 $r=R(t)\sqrt q$ 保持方程同形、无伪对流项；半径本身如何给定是核心
...
\begin{enumerate}
  ...
  \item \textbf{湿边界为声明的有效闭合}：把附件 1 的 $C_{\mathrm{env}}(t)$ 直接当作与
        ...
        该闭合是本文最大单点依赖，其不确定性由 $C_e=0.1366$ 敏感性
        $+22.6261$ h 量化，见第七章）。
  \item \textbf{热方程取无潜热的有效显热近似}（理由：
        潜热通量与显热通量同量级（通量比为 2.06--2.49，以第一问 1800 s 点的
        表面量后验复算比为 5.54）
  ...
  \item \textbf{几何}：长度不变、内部等比仿射收缩 $r=R(t)\sqrt q$
        ；$R(t)$ 由半经验收缩闭合
...
在端点取值产生内通量被强制为零的伪根），不把末单元平均值当作表面值。
...
$g_c,g_p$ 是带物理动机的经验形状函数，
本闭合是\textbf{半经验}结果；5 个参数由最小二乘对附件 2 标定
，2 个量（$C_{\mathrm{glass}}$、$\rho_{sk}=1468$）
...
存在循环；奇偶折半留出法
只检验同一光滑曲线上的\textbf{插值稳健}；
真正的独立验证需要\textbf{完整耦合标定}（每组候选参数从相同初值重解），
本文当前是借助实测几何生成湿度场再标定的近似。
...
20 h 后即进入平台，故等间隔取样、等宽排版下视觉变化前快后慢；
...
在固定初值边界下为 \textbf{14.5491} h，排除线随边界约定移动，比较序不自动成立；
