...
\usepackage{float}
% 清单代码字体：cmtt（Latin Modern Mono）缺 λ μ ε ρ ∂ ∈ ∫ ≡ ≈ ≥ ∝ ⇒ 与上/下标等
% 18 个符号，缺字会被 XeTeX 整字丢弃；JuliaMono 四种字重/字形全覆盖这些符号，
% 且 Scale=0.875 使其字宽 0.600 em × 0.875 = 0.525 em 与 cmtt 一致，
% 故代码的折行与分页不变（粗体关键词、斜体注释的原有观感也得以保留）。
...
% 行号落入框内、框线与版心对齐：xleftmargin 让出行号位，framexleftmargin 把框左线移回版心左界，
% numbersep 收窄行号与代码的间距；三者需配套，改其一都会使框线偏离版心
% showstringspaces=false：listings 默认把字符串内的空格画成可见空格符「␣」，
% 且该符号取默认 \ttfamily（cmtt），与清单字体不一致，正文代码里应按普通空格排版。
...
% 页码：页脚居中、阿拉伯数字连续编号，全篇无页眉（校内格式规范第三条）
\pagestyle{plain}

% 行距：单倍行距（校内通知四.3：其余内容小 4 号宋体、单倍行距）
\linespread{1.0}
...
\noindent 中药材干燥是产地加工的关键环节：水分偏高易霉变，干燥不足或过度都会影响品质。本文以长 25 cm、半径 2 cm 的圆柱药柱为对象，初始干基含水率 2.55 kg/kg、初温 28
${}^\circ$C，研究预热段与全过程的温度场与水分浓度场演变，并确定固定几何与计入失水收缩两种情形下的达标时间，为烘干时长的控制提供依据。

\noindent 针对问题 1（0--1800 s 预热段），含水率变化很小、热湿近似解耦，材料参数按附录 2 取常数，热方程退化为线性圆柱 Robin 问题；由此写出圆柱 Robin 特征展开与时变环境 Duhamel
...
\noindent 针对问题 2，材料参数按附录 3 取用，保留温度与水分浓度的双向耦合，用质量坐标下的守恒型有限体积格式求解：表面取半单元串联阻力，时间推进用后向
...
\noindent 针对问题 3，在问题 2 的模型上加入达标判据（各处水分浓度首次低于0.15 kg/kg 干基），由数值扫描确认中心为最慢点，按穿越区间及其右侧安全端点报告达标时刻：$\bm{t_f=57.1799}$ h（Richardson
外推 57.1636 h）。该值远大于常系数参考时标15.70 h，量级与参数一致；其中对边界汇取值最敏感，$C_e=0.1366$ 时增大 22.6 h，已检验因素的累计偏移量级约 1.71 h。

\noindent 针对问题 4，几何按仿射收缩 $r=R(t)\sqrt q$ 演化，方程与固定域同形；为确定$R(t)$，把失水收缩、毛细-膨压塌陷、结皮-成孔停滞三项机制写成半经验关系式，由湿度场逐步预测半
...
\appendix
% 附录内的二级标题单独配置：若不重定义 subsection，它会沿用导言区的
% \arabic{section}.\arabic{subsection}，而附录里 section 计数器为 5，会渲染成「5.1」
...
\subsection*{支撑材料文件列表}
支撑材料打包为 \texttt{论文.zip}（\texttt{论文/} 顶层目录，62 个文件、6 个目录，
解压后约 5.03 MB、压缩后约 3.76 MB）；下列清单与压缩包内容逐项一致。
赛题提供的原始数据（附件 1、附件 2，仓库 \texttt{01-题目/原始文件/}）按规范豁免，不在包内。

% 文件清单里文件名很长、内部几乎没有断行点，整块改用左对齐，避免连续等宽文件名撑出 Overfull；
% \fsep = 「、」+ 零惩罚断点，再给每两个文件名之间留一个换行机会
{\raggedright
\newcommand{\fsep}{、\allowbreak}
\begin{itemize}
  \item \textbf{论文/}（2）：\texttt{00-审计说明.md}\fsep \texttt{main.pdf}；
  \item \textbf{论文/论文源码/}（8）：\texttt{main.tex}\fsep \texttt{tables.tex}\fsep
        \texttt{写作说明.md}\fsep \texttt{gen\_tables.py}\fsep \texttt{check\_abstract.py}\fsep
        \texttt{check\_numbers.py}\fsep \texttt{check\_refs.py}\fsep \texttt{check\_table6.py}；
  \item \textbf{论文/代码/}（19）：\texttt{analytic\_heat.py}\fsep
        ...
        \texttt{solver\_q4.py}\fsep \texttt{run\_mech\_A.py}\fsep
        ...
        \texttt{交付口径-核对.py}；
  \item \textbf{论文/图/}（17）：\texttt{fig1\_输入数据.pdf}\fsep
  ...
  \item \textbf{论文/数据证据/}（12）：\texttt{结果总账.csv}\fsep \texttt{paper\_tables.csv}\fsep
        \texttt{交付口径.md}\fsep \texttt{q4\_endo\_eval.csv}\fsep \texttt{q4\_split.csv}\fsep
        ...
        \texttt{力学升级\_修复重标定.md}\fsep \texttt{内生收缩模型.md}；
  \item \textbf{论文/结果文件/}（4）：\texttt{result1.xlsx}\fsep \texttt{result2\_降采样.xlsx}\fsep
        \texttt{result3.xlsx}\fsep \texttt{result4.xlsx}。
\end{itemize}}

% 结论段较长且含不易断行的等宽文件名，单给一点 emergency 伸展量即可避免 Overfull
{\emergencystretch=2em
其中 \texttt{result1}、\texttt{result3}、\texttt{result4} 为原表原样入包；
\texttt{result2} 为\textbf{降采样版}：原表 \texttt{result2.xlsx} 为 1 s 时间网格
（温度与水分浓度 2 张表 × 209447 数据行 × 22 列，27.9 MB），超过支撑材料 20 MB 上限，
故按每 1800 s（30 min）一帧抽取、每帧保留完整径向剖面（21 个节点），
...
\section{核心程序代码}
本附录列出四个核心模块的完整源码，共 868 行；四份源码均由源文件直接引入，
未手工转录。其余脚本、数据与图表的完整可运行代码见支撑材料 \texttt{论文.zip} 的
\texttt{论文/代码/}（19 个脚本）与 \texttt{论文/数据证据/}。
