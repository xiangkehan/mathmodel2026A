...
\title{\textbf{中药材烘干过程的热湿耦合建模、守恒求解与六项检验}\\[2pt]
——基于质量坐标有限体积的四问统一求解}
\author{}
\date{}
...
\smallskip\noindent\textbf{验证与创新}：本文共做六项检验：问题 1 热场用解析谱解与数值解
互证；其余五项依次是尾段结构诊断、参考时标判据、守恒恒等式、退化测试与网格收敛。
这些检验用来确认所报的场值与达标时间出自该模型、量级与参数一致，而不是数值假象。
已检查因素的累计偏移量级约 1.71 h，其中数值误差、环境情景与取法敏感性三类
...
守恒有限体积加自适应时间积分，并用解析退化、守恒恒等式与网格收敛三条作多层级验证；
...
Bessel 首模，是独立对照。模型的基本假设本身不在本项检验范围内 \\
...
\begin{enumerate}
  ...
  \item \textbf{各层验证相互独立}：解析谱解与守恒有限体积是两种不同的数学结构，
        V1 给出逐点最大偏差 $5.9\times10^{-7}\,^{\circ}$C 与二阶收敛；此外还有
        退化测试、守恒恒等式（$10^{-11}$ 量级）与参考时标判据检验，各层分别给出
        可以得到的结论与尚未检验的误差源。
