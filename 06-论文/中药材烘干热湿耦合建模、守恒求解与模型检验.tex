...
\subsection*{支撑材料文件列表}
交付文件夹为 \texttt{论文/}（顶层目录，64 个文件、7 个目录，未压缩约 5.1 MB）；
...
\begin{itemize}
  \item \textbf{论文/}（7）：\texttt{中药材烘干热湿耦合建模、守恒求解与模型检验.pdf}\fsep
        \texttt{result1.xlsx}\fsep \texttt{result2.xlsx}\fsep \texttt{result3.xlsx}\fsep
        \texttt{result4.xlsx}\fsep \texttt{AI工具使用声明.docx}\fsep \texttt{00-审计说明.docx}；
