"""数字一致性自查（A12 结构化重写版）：期望值全部来自 数据 的数据文件，
中药材烘干热湿耦合建模、守恒求解与模型检验.tex / tables.tex 只作为“被核对对象”。

数据源（本模块只读，不重算）
  总账 = 数据/结果总账.csv             (类别, 项目, 子项) → 值 / 单位/备注；论文关键数字主源
  SPLIT = 数据/q4_split.csv            问题 4 效应拆分（基准＝内生几何主解）
  EVAL = 数据/q4_endo_eval.csv         耦合主解满 72 h 的三段误差等
  CONV = 数据/q4_endo_convergence.csv  问题 4 网格收敛序列、附件 2 外生版 t_f
  CAL = 数据/endogenous_calibrated.csv 半经验闭合标定（RMSE、平台、滞后交叉）
  LAT = 数据/latent_scenario.csv       潜热对照情景（# key=value 头）
  PT = 数据/paper_tables.csv           论文表 1–6 全部单元格
  Q23 = 数据/q23_summary.json          问题 3 收敛序列 / 参考时标（总账无对应键者）
  MECH = 数据/mechRefit.csv + mechRefit_fields.npz   §6.7 力学重标定
  RES = 结果/result{1,2,3,4}.xlsx      交付工作簿（md5 与行数现场核对）
...
def main():
    ...
    chk("下界 瞬时首模参考", "19.02", _led("验证", "V3问题4瞬时首模近似参考(时间积分)"),
        "总账 验证/V3问题4瞬时首模近似参考")
    lit("下界 膜界", "11.17", "数据/v3_bound.csv t_film_h（总账无键）")
    lit("下界 固定 R₀ 版", "37.61", "数据/q4_bound.csv t_r0_h（总账无键）")
    lit("下界 R_min 常值版", "15.95", "数据/q4_bound.csv t_rmin_h（总账无键）")
    ...
    lit("反演平均含水率", "0.278", "数据/内生收缩分析.md（总账无键）")
    lit("模型同刻平均含水率", "0.149", "数据/内生收缩分析.md（总账无键）")
    ...
    # ============ 收缩机制分析小节（来源：数据/内生收缩分析.md） ============
    # 2026-09-13：原锚点 "31\\%" 对应的句子在语言润色中改写为"末期孔隙率 30.9\\%"，锚点随之改指。
    for s in ("0.182 cm", "9.1\\%", "0.028,0.580", "0.129", "0.155", "30.9\\%",
              "0.234", "1.198 cm", "61\\%", "约 16 h 交叉", "-2.4\\%", "+12\\%",
              "C_{\\mathrm{glass}}\\approx0.21"):
        lit(f"收缩机制 {s}", s, "数据/内生收缩分析.md（总账无键）")

    # ============ 内生收缩模型小节 ============
    for s in ("0.1108", "1.668", "1.220", "0.0709", "3.787"):
        lit(f"收缩模型参数 {s}", s, "数据/内生收缩模型.md §参数（总账无键）")
    ...
    # 该值曾被写成 0.31 mm（偏大 10 倍），源 数据/内生收缩模型.md 与 中药材烘干热湿耦合建模、守恒求解与模型检验.tex 两处已更正
    ...
    lit("耦合主解末期孔隙率", "30.9\\%", "数据/内生收缩模型.md（总账无键）")
    lit("孔隙转正时刻", "9.5 h", "数据/内生收缩分析.md（总账无键）")
    lit("留出法区间", "0.045--0.050 mm", "数据/内生收缩模型.md §留出法（总账无键）")
    ...
    for s in ("1.4\\%,28.8\\%", "4.7\\%", "11.9\\%", "11.0\\%", "3.8\\%", "23.9\\%",
              "0.98\\%", "k_p", "存疑"):
        lit(f"参数验证 {s}", s, "数据/参数第二路线验证.md（总账无键）")
    ...
    # ============ 参考时标 / V2·V4 归位 ============
    for s in ("14.5491", "常系数参考时标", "瞬时首模近似给出的参考值"):
    ...
    for s in ("0.604", "0.212"):
        lit(f"力学 分窗误差 {s}", s, "数据/力学升级_修复重标定.md §4（总账无键）")
    for s in ("0.587/0.591", "0.91", "9.5\\times10^{-11}", "0.0084/0.0023/0.0013", "0.636",
        ...
        lit(f"力学 {s}", s, "数据/力学升级_修复重标定.md §3/§4/§6（总账无键）")
    for s in ("扩展研究", "本文的力学模型还不能复现附件 2 的实测过程"):
    ...
    # 2026-09-13：原锚点"声明的有效基线模型"、"声明闭合"随本轮语言改写，
    # 改指文中现行说法"无潜热基准"、"本文设定的取法"（§ 指向不变）。
    for s in ("无潜热基准", "本文设定的取法"):
        lit(f"潜热 定性 {s}", s, "正文 §2.3/§6.4 口径命名（总账无键）")

    # ============ 纠错修订（正文定性表述，无数据源） ============
    # 2026-09-13：以下 6 个锚点随 d1abfe0「语言润色」整段删除的"纠错修订"旁白一并退役
    # （原字面："单调映射总能构造出来" "伪影" "17.26\%" "7.33" "9.94\%" "约 31\% 固体应变"）。
    # 退役仅因该段已被有意删去；若日后把该段加回，请把这些锚点一并恢复。
    # 2026-09-13：下列 5 条字面随本轮语言改写换成文中现行说法，其余锚点未变。
    for s in ("半经验收缩关系式", "插值稳健", "完整耦合标定",
              "C_{s,\\min}=0.0488", "近平台段", "附件 2 的时间范围是 0--72 h",
              "无法检验", "其反演平均含水率为 0.278 kg/kg，低于模型同刻的 0.149 kg/kg",
              "问题 4 主模型由三项物理机制叠加而成",
              "降为验证数据", "力学驱动的推广是后续可继续沿用的方向"):
