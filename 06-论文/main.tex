...
中心最后达标；(b) 内生半径 $R(t)$ 在 50.5 h 进入近平台，60--72 h 仅再降 0.31 mm。}
\label{fig:中心表面}
\end{figure}
...
\caption{问题 4 的药柱时空演化。水分浓度与温度的
3D 时空曲面显示，干燥锋面自表面向内推进的同时外半径持续收缩（$R(t)$ 由内生模型
...
热图（色标为水分浓度）显示，干燥自表面向中心推进、外半径同步收缩并进入平台：
附件 2 实测的平台半径为 1.198 cm，模型内生预测的末端半径为 1.19 cm
（各面板标题给出该时刻的模型半径）；中心为最慢点，
$t_f=50.5369$ h 时中心首次低于 0.15 kg/kg。}
\label{fig:截面}
\end{figure}
