...
def main():
    ...
    expect_num("问题4 t_f 主值", q4["tf4_h"], "{:.4f}")
    expect_num("问题4 Richardson", q4["tf2_rich_h"], "{:.4f}")
    expect_num("附4固定R t_f", q4["tf1_fixedR_h"], "{:.4f}")
    # ---- 下界 ----
    ...
    for s in ("79.8270", "22.6261", "39.6", "58.8203", "57.5034", "50.8317",
              "0.0022", "129.1047", "78.28", "60.6", "11.1", "6.36"):
        expect(f"敏感性 {s}", s)
    # ---- 误差预算 ----
    ...
    # ---- t_f 收敛序列 ----
    for s in ("57.249/57.201/57.180", "50.838/50.830/50.823"):
        expect(f"收敛序列 {s}", s)
    ...
    for s in ("0.1108", "1.668", "1.220", "0.0709", "3.787",
              "0.047 mm", "0.162 mm", "17.0 h", "1.1969", "50.72", "-0.22\\%",
              "50.88", "+0.10\\%", "0.048 mm", "1.1967", "30.9\\%", "9.5 h",
              "0.045--0.050 mm", "0.278", "0.149"):
        expect(f"内生收缩 {s}", s)
    # ---- 两条定量声明 ----
