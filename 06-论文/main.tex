...
初始干基含水率 2.55 kg/kg）的烘干要求依次给出预热段温度与水分浓度场、全程场、
达标时间与收缩下的达标时间。挑战在于：三套附录物性必须按问题分别使用且不得调和；
扩散系数 $D$ 对局部含水率与温度强非线性（尾段变化 16.8 倍）；环境数据仅覆盖 4 h、
半径数据在约 21 h 进入近平台段——任何一处口径处理不当都会使结果偏差数十小时。

\smallskip\noindent\textbf{方法}：本文把四问统一为\textbf{一套热湿耦合方程加四个配置开关}：
从干固体守恒、水分守恒与能量守恒出发，以干物质标签 $q$ 为材料坐标建立质量坐标方程，
收缩几何下方程同形、无伪对流项；用守恒型有限体积求解（表面半单元串联阻力、
后向欧拉加局部外推、Picard--Newton 切换、自适应步长），并以题给参数构建判据族
（$Le$、$Bi_m$、$Bi_h$）与参考时标组织简化与验证。

\smallskip\noindent\textbf{关键结果}：问题 1 温度场与解析解逐点对拍最大偏差
...
附件 2 实测半径作交叉验证，偏差 $-0.56\%$）。机制归因：附录 4 与附录 3 的扩散系数比
随含水率由 $1/5.39$ 变化到 $1/2.10$；附录物性组合效应使固定半径烘干时间放大
$2.26$ 倍（$+78.57$ h），半径收缩（$R^2$ 降 64\%）加速 $60.9\%$，净效应使问题 4
反而快 $11.6\%$；敏感性分析显示边界汇口径（$C_e$）是唯一能量级翻盘的单点（$+22.6$ h）。

\smallskip\noindent\textbf{验证与创新}：六层验证体系每层明确能排除与不能排除的错误类型，
解析解与数值解是两种不同数学结构的互证；已检查因素的累计偏移量级约 1.71 h
（数值误差、环境情景、模型结构差异三类分报）。
内生收缩模型（三项物理动机机制的半经验闭合，5 个标定参数）以 RMSE 0.047 mm
贴合附件 2 全部形态（\textbf{单向标定贴合度}）；耦合主解全程 RMSE \textbf{0.088 mm}
（满 72 h 真实轨迹：0--达标 0.065 / 达标--72 h 0.127 mm）；
力学驱动扩展 RMSE 0.522 mm、$t_f=51.1667$ h，结构性上限与材料缺口定量在案。
...
\begin{enumerate}
  \item[\textbf{问题 1}] 预热平衡阶段（0--1800 s）药材内温度与水分浓度的时空分布
        （附录 2 物性），按题给表格格式交付（result1.xlsx）；
  \item[\textbf{问题 2}] 整个烘干过程的温度与水分浓度分布（附录 3 物性，result2.xlsx）；
  \item[\textbf{问题 3}] 固定几何下首次全部达标的烘干时间（表 5、result3.xlsx）；
  \item[\textbf{问题 4}] 计入失水收缩（附件 2 外半径、附录 4 物性）后的达标时间
        （表 6、result4.xlsx）。
\end{enumerate}
拟采用的总体思路：不建立四个孤立模型，而用\textbf{一套热湿耦合控制方程加四个
配置开关}（物性来源、热湿耦合、几何是否演化、输出形式）统一求解——问题 1 是
...
\caption{几何模型与边界条件（回答"求解什么问题"）：长 25 cm、半径 2 cm 的圆柱药柱，
...
四问共享同一物理过程：内部导热/导湿、表面对流边界、强非线性物性。难点有三：
\textbf{(i)} 三套附录物性（附录 2/3/4 的 $\rho,c_p,k,D$ 互不相同）必须按问题
分别使用且不得调和、平均或常数化——例如附录 4 与附录 3 的扩散系数比
$0.175\,\mathrm e^{0.15/U}$ 在高含水端约 $1/5.4$（图~\ref{fig:扩散系数}），
把任一物性常数化后结果"看起来仍合理"（仍落在 2--3 天），但违反题目指定的建模方式；
...
\caption{扩散系数非线性与特征时间尺度（回答"为什么难、量级多少"）：(a) 附录 3/4 的
$D(C,T)$ 半对数曲线——尾段（$C\to0.15$）$D$ 缩小 16.8 倍，附录 4 全程低于附录 3；
...
按附录 2 常物性）。可建立线性热传导的解析谱解（圆柱 Robin 特征展开 + 时变环境
Duhamel 卷积）作为独立参考，数值解与之逐点对拍——这是排除离散、边界、时间积分
...
（$T_a=50.165\,^{\circ}$C、$C_{\mathrm{env}}=0.04986$，对照口径进敏感性分析）；
...
\caption{输入数据（回答"数据可用性如何裁定"）：附件 1 烘房温度与空气水分浓度
...
\begin{enumerate}
  ...
  \item \textbf{逐点物性、局部瞬时取值}：$\rho,c_p,k,D$ 按附录公式在局部 $C$、
...
在端点取值产生"内通量被人为归零"的伪根），绝不把末单元平均当表面值。
...
（插值误差折 $2.6\times10^{-4}$ h，见第七章）。
非线性用外场 Gauss--Seidel（热场在冻结系数下线性求解，湿场在冻结温度下
Newton/Picard），湿场 Picard 起步、$\max_qU<0.6$ 后切换含 $\partial D/\partial C$
的 Newton（全程 Picard 对照 $\Delta t_f=1.7\times10^{-10}$ h，无感）。
全部交付由 \texttt{python run\_all.py} 一条命令产出，无人工干预；
连续两次运行 result1--4 的 md5 完全一致（附录 D 给出哈希锚点）。

\subsection{问题 1：预热平衡段的场（0--1800 s）}
\textbf{模型}：附录 2 常物性、热湿解耦，热方程为线性圆柱 Robin 问题；
湿分在 1800 s 内变化小但非零，仍按同一框架求解。
\textbf{求解与局部检验}：数值解与解析谱解（圆柱 Robin 特征展开 + 时变环境
Duhamel 卷积，稳态减法消除级数尾巴，解析参考自身误差 $<10^{-8}\,^{\circ}$C）
逐点对拍：单元中心最大偏差 $5.9\times10^{-7}\,^{\circ}$C（N=320），
收敛阶 2.00/1.99/1.99（图~\ref{fig:对拍}）——离散、边界处理、时间积分
三类实现错误被排除（物性取值错误不在此层排除之列）。
\textbf{结果}：温度与水分浓度径向分布见图~\ref{fig:温湿剖面}（时间色阶族）、
...
\caption{温度场时空演化（回答"热透过程怎样展开"）：时间 × 径向位置的温度云图
——表面在约 600 s 内贴近环境、中心滞后明显，1800 s 末全场温差收敛到
约 $1\,^{\circ}$C 量级；读数用云图与等温线，结构直观用图~\ref{fig:3d} 的三维曲面。}
\label{fig:温度云图}
\end{figure}
...
\caption{表面通量与表面扩散系数（回答"边界交换多强、何时衰减"）：(a) 表面水分
通量 $j_s$ 与表面扩散系数 $D_s=D(C_s,T_s)$ 时程——通量初期最大随后衰减，
$D_s$ 随表面变干持续下降；(b) $D_s$ 对 $C_s$ 的半对数关系，直观显示
$D\propto\mathrm e^{-0.45/C}$ 的强非线性。}
\label{fig:表面通量}
\end{figure}

\subsection{问题 2/3：全程场与固定几何达标时间}
\textbf{模型}：附录 3 物性、双向耦合、固定几何；达标判据
\begin{equation}
...
\textbf{结果（场）}：温湿剖面演化（viridis 时间色阶）见图~\ref{fig:温湿剖面}，
...
\caption{温湿剖面演化（回答"场长什么样"）：代表时刻的径向温度/水分浓度剖面
（viridis 时间色阶）——表面先干、中心最慢、尾段拖长。}
\label{fig:温湿剖面}
\end{figure}
...
\caption{水分场时空演化（回答"干壳/湿核如何形成"）：时间 × 径向位置的水分浓度
云图——干燥锋面内移、表面快速进入低 $D$ 区形成干壳，中心湿核维持到 40 h 以后；
等值线给出 0.15 kg/kg 达标线，中心最后穿越。}
\label{fig:水分云图}
\end{figure}
...
与药材水分限度的通用口径（约 13\%，未核对具体药典条文与版本卷次）相符~\cite{pharmacopoeia}。
...
干物质密度，$L_v=2.4\times10^6$ J/kg 近似值），同一附录 3 物性、同一边界数据、
...
低于湿球温度，正暴露 $h_m$ 与能量边界的不协调），情景值不进主交付口径。
...
要如实说明：$g_c,g_p$ 是带物理动机的经验形状函数、非应力平衡推出，
...
非拟合量）复现附件 2——RMSE 0.047 mm（量化分辨率的 5 倍）、滞后交叉
（17.0 h vs 约 16 h）、平台（1.1969 vs 1.198 cm）、三段形态全对；
残差子图显示偏差有界在 $\pm0.16$ mm（单向拟合口径）。}
\label{fig:内生贴合}
\end{figure}
...
60--72 h 再降 0.31 mm（末端近似平台但有残余缓降，与结构限定一致）。
半径预测所用的代数闭合残差（在 $R_{\mathrm{ideal}}$ 下求 $C_s$ 再代入修正式的
未收敛残差）max 0.003 mm；末单元温度与重建表面温度之差 max 0.018 ${}^\circ$C
——对 $t_f$ 影响远在 0.01 h 以下。
收敛序列 50.5450/50.5370/50.5369 h 的相邻差（28.8 s、0.41 s）后者已小于
尾段约 15 s 的步宽，表观收敛阶不作数；改用连续事件定位的 $t^*$ 序列
50.5422/50.5355/50.5362 h（N=80→160 变化 0.0007 h 作空间误差量级，见第七章）。
中心/表面演化与半径收缩见图~\ref{fig:中心表面}，三维时空演化与截面演化见
图~\ref{fig:3d}、图~\ref{fig:截面}；交付表见表~\ref{tab:表6}（result4.xlsx）。

\begin{figure}[htbp]
\centering
\includefig[\linewidth]{fig16_中心表面演化与半径收缩.pdf}{中心/表面演化与半径收缩}
\caption{中心/表面含水率演化与半径收缩（回答"达标过程如何推进"）：(a) 中心与表面
含水率曲线 + 0.15 kg/kg 阈值虚线 + $t_f=50.5369$ h 达标竖线——表面先到干态、
中心最后达标；(b) 内生半径 $R(t)$：50.5 h 进入近平台，60--72 h 仅再降 0.31 mm。}
\label{fig:中心表面}
\end{figure}
...
\caption{问题 4 的药柱时空演化（回答"收缩下的场长什么样"）：水分浓度与温度的
3D 时空曲面——干燥锋面自表面向内推进的同时外半径持续收缩（$R(t)$ 由内生模型
预测，附件 2 降为验证数据）；定量读数用图~\ref{fig:水分云图}，结构直观用本图。}
\label{fig:3d}
\end{figure}
...
\caption{问题 4 的截面演化（回答"达标过程如何推进"）：6 个代表时刻的径向剖面
热图，干燥自表面向中心推进、外半径同步收缩至平台 1.198 cm 附近
（内生预测 1.19 cm）；中心为最慢点，$t_f=50.5369$ h 时中心首次低于 0.15 kg/kg。}
\label{fig:截面}
\end{figure}

\textbf{与问题 3 的大小关系}（三组对照，机制归因必须定量）：
\begin{itemize}
  \item \textbf{附录物性组合效应}：附录 4 与附录 3 的 $D$ 之比
        $0.175\,\mathrm e^{0.15/U}$ 由 $1/5.39$（$U=2.55$）变到 $1/2.10$（$U=0.15$）；
        固定半径下 $t_f$ 从 57.18 h 放大到 129.10 h（$\times2.26$，$+78.57$ h）——
        该对照同时改变了全部热物性，不能严格归因于单个扩散系数倍率。
  \item \textbf{收缩效应}：$R^2$ 降 64\%（扩散时间尺度缩至 0.36）、$2/R$ 换热换质
        面积比增 67\%——把 129.10 h 拉回 50.54 h（$-60.9\%$），
        收缩是本问题最大的单点加速因素。
  \item \textbf{净效应}：物性组合减速（$\times2.26$）被收缩加速反超，
...
本节的力学模型是严谨骨架，用途是讲清收缩的机制结构与适用边界、定量定位走向
物理预测所缺的数据。缺保水与力学实测数据，结论限于机制结构与情景分析。
...
\textbf{标定与复现程度（如实）}：幂律保水—骨架结构标定到附件 2
...
\caption{力学驱动内生收缩模型（回答"半径能否由材料变形求出"）：修复后求解器 +
幂律保水重标定（$\beta=0.106$）对附件 2 的拟合——RMSE 0.522 mm
（残差三段：急缩 1.193 / 缓缩 0.875 / 平台 0.163 mm）、$t_f=51.1667$ h；
末端 60--72 h 仅再降 0.04 mm（真平台），平台机制为吸力饱和平衡
（De $\approx10^{-3}\ll1$）。右图：气相体积份额全程非负（$g_{\min}=0.1446$）。}
\label{fig:力学升级}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{$Bi_m$ 区间 $\gtrsim O(1)$ 表明内部扩散主导}：集总模型给出
        $t_f=11.17$ h，比全耦合解快 $46.03$ h（$-80\%$）——任何"集总/平均
...
两点纪律：(i) V1 与 V5 的 $D$ 冻结档是解析结构对有限体积的\emph{真独立}对拍
（不是换参数、不是换网格；剖面对拍见图~\ref{fig:对拍}）；V2 为后验尾段诊断，
...
V3 参考时标闸门（常系数/瞬时首模近似，见 6.2 节限定） &
...
\begin{figure}[htbp]
\centering
\includefig[\linewidth]{fig2_问题1解析对拍.pdf}{问题 1 解析对拍}
\caption{问题 1 解析对拍（回答"可靠性如何证明"）：特征位置（中心/表面）温度的
数值解与解析谱解时程（V1）——逐点最大偏差 $5.9\times10^{-7}\,^{\circ}$C；
偏差随网格的二阶收敛（斜率参考线）证明实现无误。}
\label{fig:对拍}
\end{figure}
...
\caption{收敛与守恒检验（回答"离散与实现可信吗"）：(a) 收敛性——解析对拍偏差
随网格的二阶下降（V1/V6）与 $t_f$ 收敛序列；(b) 守恒性——水量与显热离散平衡
残差全程维持 $10^{-11}$ 量级（V4）。}
\label{fig:收敛守恒}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includefig[\linewidth]{fig4_达标穿越.pdf}{达标穿越}
...
\subsection{敏感性分析}
每组实测 $\Delta t_f$（不只报方向），见表~\ref{tab:敏感性} 与图~\ref{fig:敏感}。
边界汇口径（$C_e$）是本题\emph{唯一能量级翻盘的单点}：改用解吸等温线
$C_e=0.1366$ 后 $t_f$ 增大 $22.6$ h（$39.6\%$）——按纪律，主结果一律用声明
闭合口径，题外修正只入敏感性并明确标注；该偏移不进误差预算（7.2 节）。
...
\begin{table}[htbp]
\centering
\caption{误差预算表（三类分报、两问分列；$C_e$ 口径与潜热情景属模型结构差异，
单列不合成）。}
\label{tab:预算}
\small
...
边界汇口径 $C_e=0.1366$ $+22.6261$ h（题外修正，单列敏感性）。
...
\begin{enumerate}
  ...
  \item \textbf{验证分层且独立}：解析谱解与守恒有限体积两种不同数学结构互证
        ...
        守恒恒等式（$10^{-11}$ 量级）与参考时标闸门，每层明确能/不能排除的错误类型。
  \item \textbf{收缩由模型内生预测}：问题 4 不读附件 2 而由半经验闭合逐步预测
        $R(t)$（$t_f=50.5369$ h），附件 2 降为验证数据后仍全程吻合
        （0.088 mm），两个精度口径与标定循环的局限均已定量声明。
  \item \textbf{不确定度透明}：判据族反验、敏感性全 $\Delta t_f$、误差三类分报、
...
\begin{enumerate}
  ...
  \item 针对缺点 3：补测真实保水曲线 $p_c(S,T)$ 与骨架模量/松弛谱，
        沿力学驱动路线（5.5 节骨架已在案）重新标定，半经验闭合作为低维验证基准；
...
(ii) 固定半径退化测试（V5a）\textbf{检验不了}这个缺口——
...
参数 & 取值/公式 & 取值方式 & 适用 \\
\midrule
$D(C)$ & $1.7\times10^{-3}\mathrm e^{-0.28/C}$ & 局部 $C$ & 问题 1 \\
