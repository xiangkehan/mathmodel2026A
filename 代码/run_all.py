...
依次执行 run_q1 → run_q23 → run_q4 → **run_q4_endo**（数据：问题 4 内生几何主解，
产出内生版 result4.xlsx）→ **run_q4_endo_fields**（数据：内生时空场 q4_endo_fields.npz）
→ run_m4（判据/敏感性/误差/总账），各自的一条命令产物与门禁全部包含，
日志落 ../数据/run_all.log；结束打印 result1–4 的 md5 供两次运行比对。
...
DATA_DIR = A_DIR / "数据"
sys.path.insert(0, str(DATA_DIR))            # run_q4_endo*.py 在 数据（活依赖链）
...
def main():
    ...
    import run_q23
    import run_q4
    import run_q4_endo            # 数据：问题 4 内生几何主解（→ 内生版 result4.xlsx）
    import run_q4_endo_fields     # 数据：内生时空场（→ q4_endo_fields.npz）
    import run_m4
