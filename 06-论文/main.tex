...
初温 28 ${}^\circ$C、初始干基含水率 2.55 kg/kg，要求给出预热段与全过程的温度场与
水分浓度场、固定几何下的达标时间，以及计入收缩后的达标时间。三套附录材料参数须按
问题分别使用，不得把三套参数调和、平均或取为常数；扩散系数 $D$ 的非线性强，
尾段最多缩小 16.8 倍；环境数据只有 4 h，远短于干燥历时；
半径数据在约 21 h 后进入近平台段。

\smallskip\noindent\textbf{方法}：四问共用\textbf{同一套热湿耦合方程，差别只在取法}：
由干固体守恒、水分守恒与能量守恒导出，以干物质标签 $q$ 为材料坐标，写成质量坐标
形式，收缩几何下形式不变，且无伪对流项；求解用守恒型有限体积格式，表面取半单元
串联阻力，时间推进用后向欧拉加局部外推，非线性项用 Picard--Newton 交替迭代，
步长自适应；并以题给参数构造三个无量纲判据 $Le$、$Bi_m$、$Bi_h$ 与参考时标，
用于简化与验证。

\smallskip\noindent\textbf{关键结果}：问题 1 温度场与解析解逐点比较，最大偏差为
$5.9\times10^{-7}\,^{\circ}$C；问题 3 达标时间 $\bm{t_f=57.1799}$ \textbf{h}，
大于常系数参考时标 15.70 h，Richardson 外推给出 57.1636 h；问题 4 达标时间
$\bm{t_f=50.5369}$ \textbf{h}，大于瞬时首模近似参考 19.02 h，其几何由内生收缩模型
预测，附件 2 实测半径作交叉验证，偏差 $-0.56\%$。机制归因：附录 4 与附录 3 的扩散
系数比随含水率由 $1/5.39$ 变化到 $1/2.10$；附录材料参数组合效应使固定半径烘干时间
放大 $2.26$ 倍，相当于 $+78.57$ h，半径收缩使 $R^2$ 降低 64\%，加速 $60.9\%$，
两者相抵后问题 4 反而快 $11.6\%$；敏感性分析显示边界汇取值 $C_e$ 是唯一能改变
结论量级的因素，其影响为 $+22.6$ h。

\smallskip\noindent\textbf{验证与创新}：问题 1 的热场与解析谱解逐点一致；此外还做了尾段结构诊断、
参考时标判据、守恒恒等式、退化测试与网格收敛检验。
这些检验用来确认所报的场值与达标时间出自该模型、量级与参数一致，而不是数值假象。
已检验因素的累计偏移量级约 1.71 h，其中数值误差、环境情景与取法敏感性三类
分别报告，模型结构差异单列。内生收缩模型把三项有物理含义的机制写成半经验关系式，含 5 个
标定参数，均由附件 2 的实测半径标定，与附件 2 全部形态的 RMSE 为 0.047 mm。耦合主解在
满 72 h 的真实轨迹上全程 RMSE \textbf{0.088 mm}，其中 0--达标段 0.065 mm、
达标--72 h 段 0.127 mm。力学驱动扩展模型 RMSE 0.522 mm、$t_f=51.1667$ h，
其偏差来自模型结构决定的上限与材料参数缺失，两者已按数据说明。

\smallskip\noindent\textbf{关键词：} 热湿耦合\quad 质量坐标\quad 守恒有限体积\quad 无量纲判据\quad 模型检验\quad 中药材干燥
\end{abstract}
...
\begin{enumerate}
  ...
  \item[\textbf{问题 2}] 整个烘干过程的温度与水分浓度分布情况，材料参数取附录 3；
  \item[\textbf{问题 3}] 固定几何下首次全部达标的烘干时间，结果按表 5 的格式给出；
  \item[\textbf{问题 4}] 计入失水收缩后的达标时间，外半径取附件 2、材料参数取附录 4，
        结果按表 6 的格式给出。
\end{enumerate}
...
边界汇取值（$C_e$）是本题\emph{唯一能改变结论量级的因素}：改用解吸等温线
...
本文在竞赛过程中使用了 \textbf{KimiCode} 与 \textbf{DeepSeek} 两个 AI 工具，用途限于以下三项，均为辅助性使用：AI 给出的代码、图形与推导结论均由作者复核、复算后才写入论文，论文的建模思路、结果与结论由作者负责。

\begin{itemize}
  \item \textbf{程序测试}：辅助编写与调试数值求解代码，以及守恒恒等式、网格与时间收敛、退化测试等检验脚本。涉及第 5 章（模型建立与求解）、第 6 章（模型检验）与附录 D。
  \item \textbf{绘图}：辅助整理数据并生成全文插图。涉及第 1--7 章的插图，图形内容与图中数值标注由作者核对。
  \item \textbf{数学公式推理验证}：辅助核对控制方程与移动域推导、解析谱解、量纲与极限退化。涉及第 5.1 节、附录 A 与第 6.1--6.2 节。
\end{itemize}
